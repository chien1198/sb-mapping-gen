...
Nguồn: `input/DATAMODEL_DWH_LOS_20260907.xlsx`. Tổng hợp từ 3 sheet giải thích
...
| 21 | FCT_LOS_WORKSTEP_EVENT | Bảng nền | Cấu trúc dữ liệu nguồn | 1 dòng = 1 phiên bản của 1 logical event, trong đó logical event = 1 hồ sơ x 1 workstep x 1 lần vào bước (ENTRYDATE). Hồ sơ quay lại cùng một bước (ví dụ bước thứ 3 trong workstep) thì mỗi lần vào lại là một logical event/dòng riêng, không gộp chung với lần trước | Cả CLOS và RLOS chạy trên cùng một workflow engine, ghi nhật ký vào NG_SB_*_ENTRY_EXIT: mỗi lần hồ sơ vào một bước là một dòng, kèm thời điểm vào, thời điểm ra, người xử lý và quyết định. Đây là bảng duy nhất ở hệ nguồn mang chiều thời gian thật. Toàn bộ chỉ tiêu về mốc thời gian, thời gian xử lý, số lần trả về và số nhân sự đều tính từ bảng này. | BC1, BC2, BC3, BC4, BC5, BC7, BC8, BC9, BC10, BC11 |
...
|---|------|----------------|----------------------|------------|---------------------|-------------------|
| 1 | A | Bản ghi cờ D của 18 bảng gắn toàn bộ cột không CLOB làm khóa có chở đủ giá trị cũ của mọi cột khóa không | Khóa dòng của nhóm này là hash toàn bộ cột không phải CLOB. Không tính được hash từ bản ghi D thì không gỡ được dòng cũ khỏi ảnh hôm qua, ảnh chỉ có thêm mà không bao giờ bớt. Cả 18 bảng hỏng hoàn toàn chứ không phải hỏng một phần. | Đội CDC | Chốt được nhánh gỡ dòng của quy trình A2 | 00_Doc_STG_LOS |
| 2 | A | D và I của một lần sửa có nằm chung một COMMIT_SCN không, và GoldenGate có đưa về được một cột thể hiện thứ tự thao tác trong giao dịch không | Nếu chung COMMIT_SCN thì không xếp thứ tự được, phải dùng luật gộp-trong-commit ở 00_Doc_STG_LOS. Có cột thứ tự thì xếp thẳng, đơn giản hơn và không cần giả định nào. | Đội CDC | Chốt được bước 2 và 3 của quy trình A1 | 00_Doc_STG_LOS |
...
| 4 | A | Supplemental log group chứa được tối đa bao nhiêu cột, và redo tăng bao nhiêu sau khi bật cho 18 bảng gắn toàn bộ cột làm khóa | NG_SB_RLOS_COL_REALESTATE có 46 cột, NG_SB_RLOS_CBS 38, NG_SB_RLOS_REPAY_CALC 36. Vượt giới hạn hoặc nguồn không chịu nổi redo thì phải chọn một tập cột nhỏ hơn làm khóa, và lúc đó cần người nghiệp vụ chỉ ra tổ hợp nào định danh được một dòng. | Đội CDC + DBA | Chốt được cách gắn CDC cho 18 bảng không khai khóa | 00_Phan_loai_nguon |
| 5 | B | Cột CLOB bị loại khỏi khóa hash. Hai hệ quả: hai dòng khác nhau CHỈ ở nội dung CLOB sẽ ra cùng khóa và bị gộp; và một lần sửa chỉ chạm vào CLOB sẽ không đổi khóa nên cặp D + I đụng nhau trong cùng một commit | Ảnh hưởng 7 bảng có CLOB. Trường hợp thứ hai được luật gộp-trong-commit xử lý đúng, nhưng trường hợp thứ nhất thì chưa có cách. Ngoài ra GoldenGate có bắt và chuyển đủ nội dung LOB không, và nội dung thật có vượt VARCHAR2(4000) không. | BA/DEV LOS + Đội CDC | Biết được có cần cột phụ để phân biệt hay chấp nhận gộp | 00_Doc_STG_LOS |
...
| 13 | IS_FTR_VIOLATION | DIM_LOS_EXCEPTION_REASON | 'Y' nếu EXCEPTION_CODE chứa hậu tố 'FTR'. Theo giải thích của BA, mã dạng UW-BR-FTR nghĩa là bước UnderwriterMaker chuyển hồ sơ về BranchSupport và điều kiện trả về này thuộc danh mục vi phạm First Time Right. Đưa quy tắc CHECK_FTR của BC7 về dữ liệu | (đầu vào BC7.CHECK_FTR) |
